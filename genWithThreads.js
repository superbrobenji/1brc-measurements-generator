...
if (isMainThread) {
  ...
  const threadCount = 8
  const range = Math.ceil(maxMeasurements / threadCount);
  for (let i = 0; i < threadCount - 1; i++) {
    threads.add(new Worker(__filename, { workerData: { range, stations: stations.slice(0, Math.ceil(stations.length / 2)) } }));
  }
  threads.add(new Worker(__filename, { workerData: { range: range + ((maxMeasurements) % threadCount), stations: stations.slice(Math.ceil(stations.length / 2)) } }));
